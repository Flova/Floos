...
if(isset($_POST['pinnwand']) && $_POST['pinnwand'] != "" && $_POST['pinnwand'] !="Dein Status")
{
	...
	$insert = mysql_query("INSERT INTO pinnwand VALUES('','{$pw1}','{$uid1}','{$uid}','{$date}')");
}
                ...
                <div class="container">
                        ...
                        <nav class="collapse navbar-collapse bs-navbar-collapse" role="navigation">
                          ...
                          <ul class="nav navbar-nav navbar-right">
                            ...
                            <li>
                            	 <a href="profile.php?p=<?php $homeid2 = mysql_query("SELECT * FROM  profile WHERE administraedFrom = {$_SESSION['userid']} AND type = 1"); $homeid12 = mysql_fetch_assoc($homeid2); echo $homeid12['id']; ?>">Mein Profil</a>
                            </li>
                            <li>
...
<!-- /btn-group -->
        ...
        $selctCHData = mysql_query("SELECT * FROM `Chat` WHERE `secondID` = " . $myId . " AND `read` = 0 ORDER BY time DESC");
        if(mysql_num_rows($selctCHData) > 0){
           echo "<div class='panel panel-default'><div class='panel-heading'><b> Du hast " . mysql_num_rows($selctCHData) . " neue Chatnachicht(en)</b></div><div class='panel-body'>";
           while($newchatdata = mysql_fetch_assoc($selctCHData))
           {
              	$selectsmFrom = mysql_query("SELECT * FROM user WHERE id = {$newchatdata['firstID']}");
           ...
           echo "</div></div>"; 
						 ...
						 //Profilbild ausgabe
							$p33 = mysql_query("SELECT * FROM profile WHERE administraedFrom = {$_SESSION['userid']} AND type = 1 ");
							$p43 = mysql_fetch_assoc($p33);
							$result = mysql_query("SELECT * FROM `profileimages` WHERE 	userid = " . $p43['id']);
		            	...
		            	<?php 
						// Profil anzeigen
						$p3 = mysql_query("SELECT * FROM profile WHERE administraedFrom = {$_SESSION['userid']} AND type = 1 ");
						$p4 = mysql_fetch_assoc($p3);
						...
						$my_profil_info = mysql_query("SELECT * FROM profile WHERE id = {$p4['id']}");
						$my_data = mysql_fetch_assoc($my_profil_info);
						echo $my_data['profilInfos']."</div></div>";
			            ?>	
		            </div>
					<div class="col-md-6" >
						<?php
    						$ms = mysql_query("SELECT * FROM message WHERE userId = {$_SESSION['userid']} AND activ = 1 ORDER BY id DESC");
							if(mysql_num_rows($ms) > 0){
								$ms = mysql_fetch_assoc($ms);
					...
					<?php
					//neusten fleeses
					$select11Frinds = mysql_query("SELECT * FROM `friendship` WHERE `confired` = 1 AND `firstid` = '{$_SESSION['userid']}'");
					$np = "( " .$_SESSION['userid'] ;
					while($frinds11 = mysql_fetch_assoc($select11Frinds))
					{
						$np = $np . " , " . $frinds11['secondid'] ;                                                                
					}
					$select21Frinds = mysql_query("SELECT * FROM `friendship` WHERE `confired` = 1 AND `secondid` = '{$_SESSION['userid']}'");		
		            while($frinds21 = mysql_fetch_assoc($select21Frinds))
					{
						$np = $np . " , " . $frinds21['firstid'] ;
					}
					$np = $np ." )";
		                                                      	$selctPWData = mysql_query("SELECT * FROM  pinnwand WHERE userid IN {$np} ORDER BY poston DESC LIMIT 0 , 30 ");
						while($pwData = mysql_fetch_assoc($selctPWData))
						{
							...
							$lock = mysql_query("SELECT * FROM `locked` WHERE `pageId` = {$pwData['postOnUserID']} AND ok = 1");
							if(mysql_num_rows($lock) != 0) {
								$lock2 = mysql_query("SELECT * FROM locked WHERE pageId = {$pwData['postOnUserID']} AND member = {$_SESSION['userid']} AND ok = 1");
